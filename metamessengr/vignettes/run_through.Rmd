...
```{r}
me_cut <- mess_cut_off(me_clean, time = "2020-03-01")
```
...
We can also plot the density of the sentiment scores before and after the event using `plot_mess_dens()`. The default for this plot is to facet wrap by sender, but the function argument `wrap_by=` allows one to change the facet wrap variable.

```{r}
plot_mess_dens(me_sent)
```
